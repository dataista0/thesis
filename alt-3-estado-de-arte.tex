...
\begin{enumerate}
  ...
  \item Cada \tradqd mapeada a un atributo $t_a$ corresponde a una única \tradqd mapeada a un valor $t_v$. Esto significa:
    ...
    \end{itemize}
  \item Cada \tradqd mapeada a una relación en $D$ ($t_r$) corresponde con una \tradqd de atributo ($t_a$) o bien con una \tradqd de valor ($t_v$). Esto significa:
      \begin{itemize}
      \item La relación en $D$ a la que mapea $t_r$ y el elemento en $D$ al que mapea $t_a$ (o $t_v$) son compatibles 
      \item $t_r$ y $t_a$ (o $t_v$) están sintácticamente relacionados en el parse-tree de $q$
    \end{itemize}
\end{enumerate}

Finalmente, una pregunta $q$ se dice \textbf{semánticamente tratable} en el contexto de una base de datos $D$ si existe un mapeo válido con qtokens distintos y en el que al menos uno de sus qtokens incluye una qword.

{\color{red} Falta cerrar mejor, ejemplificar más y señalar los límites}.
Esta definición asumede tratabilidad semántica a la que llegaremos en breve se basa en la observación de que muchas preguntas expresadas en lenguaje natural especifican pares de atributos y valores o especifican solo valores, dejando el atributo implícito.
